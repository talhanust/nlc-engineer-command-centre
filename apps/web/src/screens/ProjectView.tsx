...
import { HrAuthorizationCard } from '../components/HrAuthorizationCard';
import { useRole } from '../state/Role';
import { appointment as apptOf } from '../domain/appointments';
...
export function ProjectView({ nodeId }: { nodeId: string }) {
  ...
  // Spec §2 step 5: until HR is approved, the project stays CLOSED to project staff.
  const myLevel = user?.appointmentId ? apptOf(user.appointmentId)?.level : undefined;
  if (project.hrApproved === false && myLevel === 'project') {
    return (
      <section className="card" role="alert" aria-label="Project locked" style={{ padding: 24 }}>
        <h3>Project not yet opened</h3>
        <p className="muted">HR authorisation for this project is pending on the approval ladder. The project level opens to project staff once it is approved.</p>
      </section>
    );
  }
  ...
  return (
    <section>
      ...
      {active === 'executive' && (
        <>
          <HrAuthorizationCard projectId={nodeId} hrApproved={project.hrApproved} onApproved={() => refresh()} />
          <section className="card" style={{ marginBottom: 12 }}>
            <div className="section-head"><h3>Inputs to command (proper channel)</h3>
              <span className="muted small">recorded minutes to your immediate superior — acknowledgement required</span>
            </div>
            <MarkInputPanel node={node} projectIds={[nodeId]} />
          </section>
