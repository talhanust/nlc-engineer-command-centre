...
import { MarkInputPanel } from '../components/MarkInputPanel';
...
export function CommandDashboard({ nodeId }: { nodeId: string }) {
  ...
  return (
    <section>
      ...
      <div className="print-only brief-head" aria-hidden="true">
        <div className="brief-rule" />
        <h1 style={{ margin: '4px 0' }}>MONTHLY COMMAND BRIEF — {node.name.toUpperCase()}</h1>
        <p style={{ margin: '2px 0' }}>
          Engineer Command Centre · as at {new Date().toLocaleDateString('en-GB', { day: '2-digit', month: 'short', year: 'numeric' })}
          {user ? ` · prepared for ${user.name}` : ''} · {underCommand.length} projects under command
        </p>
        <div className="brief-rule" />
      </div>
      <div className="screen-head">
        ...
        <div className="head-tools">
          ...
          <button className="btn-ghost no-print" onClick={() => window.print()}>Print command brief</button>
          <RagThresholds />
          ...
          <tbody>
            {children.map((c) => (
              <tr key={c.id} className="row-link" onClick={() => navigate(`/node/${c.id}`)}>
                ...
                <td className="num">
                  {(() => {
                    // Stage-aware: schedule health is meaningless after TOC — show the recovery position instead.
                    const proj = projects.find((p) => p.id === c.id);
                    const st = proj?.stage ?? 'ongoing';
                    if (st === 'physically_completed') {
                      const pct = c.billed > 0 ? Math.round((c.received / c.billed) * 100) : 100;
                      return <span className="status-pill st-ack" title="Physically completed — receivable collection" aria-label={`Recovery ${c.name}`}>recv {pct}%</span>;
                    }
                    if (st === 'financially_closed') {
                      return <span className="status-pill st-resolved" aria-label={`Closed ${c.name}`}>closed</span>;
                    }
      ...
      <CollapsibleCard id="dash-markinput" title="Inputs to command (proper channel)" focusable dockable>
        <MarkInputPanel node={node} projectIds={underCommand.map((p) => p.id)} />
      </CollapsibleCard>
