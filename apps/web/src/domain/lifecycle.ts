 ...
 * receivable is collected, liabilities are cleared AND no DLP defect
 * remains open. */
export function readyToClose(p: Project, liabilitiesTotal: number, openDefects = 0): boolean {
  return projectStage(p) === 'physically_completed' && receivable(p) <= 0 && liabilitiesTotal <= 0 && openDefects <= 0;
}
