...
interface ActingUser { name: string; nodeId: string }
...
interface RoleCtx {
  ...
  setUser: (u: { name: string; role: AppRole; nodeId: string } | null) => void;
...
export function RoleProvider({ children }: { children: ReactNode }) {
  ...
  const setUser = (u: { name: string; role: AppRole; nodeId: string } | null) => {
    if (u) {
      setUserState({ name: u.name, nodeId: u.nodeId });
      setRole(u.role);
      try { localStorage.setItem(USER_KEY, JSON.stringify({ name: u.name, nodeId: u.nodeId })); } catch { /* ignore */ }
    } else {
      setUserState(null);
