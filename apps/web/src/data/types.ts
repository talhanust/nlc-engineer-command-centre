...
export interface Project {
  ...
  actualPct: number;
  archived?: boolean;
  /** False until the project's HR authorisation is approved (spec §2 step 5).
   * Undefined on legacy/seeded projects = treated as approved. */
  hrApproved?: boolean;
...
export interface Rar {
  ...
  isFinal?: boolean;
  /** Appointment approval chain (spec §5) — present once submitted; final act pays. */
  chain?: import('../domain/apptchain').ApptChainState;
  chainStage?: number;
...
export interface AppUser {
  ...
  /** Appointment id from the catalogue (Requirements v2 §1) — individual logins for all. */
  appointmentId?: string;
}

/** Project HR authorisation proposal (spec §2 steps 3–5, A3): Manager HR
 * (HQ PD) drafts key appointments; the ladder delegates by grade — Gr 1–16
 * end at Comd Engrs, Gr 17+ and the overall TOHR end at DG NLC. Approval
 * opens the project level to project staff. */
export interface HrProposalEntry {
  title: string;          // 'Senior Project Manager', 'Site Engineer' …
  appointmentId?: string;
  grade: number;          // NLC grade (17+ escalates to DG)
  auth: number;           // authorised seats
}
export interface HrProposal {
  id: string;
  projectId: string;
  kind: 'key_appointments' | 'tohr';
  entries: HrProposalEntry[];
  status: 'draft' | 'in_chain' | 'approved';
  chain?: import('../domain/apptchain').ApptChainState;
  createdBy: string;
  createdAt: string;
}

/** Mark-Input (spec §9 A9): a recorded minute from a subordinate travelling
 * UP the proper channel. The superior MUST acknowledge; acknowledgement is
 * tracked and surfaces in the superior's work-list. */
export interface MarkInput {
  id: string;
  fromUser: string;
  fromAppointmentId?: string;
  /** Recipient appointment id (immediate superior on the proper channel). */
...
/** Defect raised during the Defect Liability Period (between TOC and
 * financial close). Open defects block financial closure. */
...
export interface DataProvider {
  ...
  /** Contract approval ladder (spec §4): submit builds the value-routed chain; act/return/resubmit per A2/A5. */
  submitContractApproval(projectId: string, contractId: string, by: string): Promise<Contract>;
  /** RAR ladder (spec §5): fixed steps incl. Pre-Audit; final act marks paid. */
  submitRarApproval(projectId: string, rarNo: string, by: string): Promise<Rar>;
  actOnRar(projectId: string, rarNo: string, by: string, remarks?: string): Promise<Rar>;
  returnRar(projectId: string, rarNo: string, by: string, remarks: string): Promise<Rar>;
  resubmitRar(projectId: string, rarNo: string, by: string): Promise<Rar>;
  actOnContract(projectId: string, contractId: string, by: string, remarks?: string): Promise<Contract>;
  returnContract(projectId: string, contractId: string, by: string, remarks: string): Promise<Contract>;
  resubmitContract(projectId: string, contractId: string, by: string): Promise<Contract>;
  listHrProposals(projectId: string): Promise<HrProposal[]>;
  createHrProposal(projectId: string, input: { kind: HrProposal['kind']; entries: HrProposalEntry[]; by: string }): Promise<HrProposal>;
  submitHrProposal(projectId: string, id: string, by: string): Promise<HrProposal>;
  actOnHrProposal(projectId: string, id: string, by: string, remarks?: string): Promise<HrProposal>;
  returnHrProposal(projectId: string, id: string, by: string, remarks: string): Promise<HrProposal>;
  resubmitHrProposal(projectId: string, id: string, by: string): Promise<HrProposal>;
  ...
  listMixDesigns(projectId: string): Promise<import('../domain/mixdesigns').MixDesign[]>;
  upsertMixDesign(projectId: string, design: import('../domain/mixdesigns').MixDesign): Promise<import('../domain/mixdesigns').MixDesign[]>;
